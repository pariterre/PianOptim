...
with open("/home/alpha/pianoptim/PianOptim/Nov.Codes/Updated_BioModFile_YeadonModel/Results/Felipe_25March/26March-qdot2/4/Struck_without_Thorax.pckl", "rb") as file:
...
z_contributions_by_dof = np.random.randn(num_nodes, num_dofs)

for i in range(num_dofs):

    z_contributions_by_dof[i] = detailed_contributions[i][2]

num_nodes, num_dofs = z_contributions_by_dof.shape
# Calculate the final Z velocity for each node by summing the contributions across all DOFs
final_z_velocity_per_node = np.sum(z_contributions_by_dof, axis=1)
...
for node_idx in range(num_nodes):
    contributions = z_contributions_by_dof[node_idx, :]
    axs[node_idx].bar(range(num_dofs), contributions, color=np.where(contributions > 0, 'g', 'r'))
    axs[node_idx].set_title(f'Node {node_idx + 1} - Final Z Velocity: {final_z_velocity_per_node[node_idx]:.3f}')
